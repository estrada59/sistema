        ...
        ';
                    ...
                    $sqltest = $mysql->query($link,"SELECT  t1.idpacientes,
                                                            ...
                                                            (SELECT count(idpaciente) 
                                                                FROM urgencias
                                                                WHERE idpaciente = t1.idpacientes) as urgencia,
...
function resultados_pacientes_del_mes($fecha_ini, $fecha_fin, $pagina){
            ...
            <table class="table table-bordered table-hover table-striped">
                <thead>
                    <tr>
                        ...
                        <!--<th data-field="price">Médico tratante</th>-->
                        <th data-field="estatus">Estatus</th>
                        <th data-field="estatus">No. placas</th>
                        <th data-field="estatus">No. discos</th>
                        <th data-field="estatus">Interpretación</th>
                        <th data-field="estatus">Estatus resultados</th>
                    </tr>
                </thead>
                <tbody>
        ';
                    ...
                    $sqltest = $mysql->query($link,"SELECT  t1.idpacientes,
                                                            ...
                                                            (SELECT no_discos
                                                                FROM  interpretacion
                                                                WHERE idpaciente = t1.idpacientes)as discos,
                                                            
                                                            (SELECT no_placas
                                                                FROM  interpretacion
                                                                WHERE idpaciente = t1.idpacientes)as placas,
                                                            
                                                            (SELECT interpretacion
                                                                FROM  interpretacion
                                                                WHERE idpaciente = t1.idpacientes)as interpretacion,
                                                            
                                                            (SELECT estatus_resultado
                                                                FROM  interpretacion
                                                                WHERE idpaciente = t1.idpacientes)as estatus_resultado
                    ...
                    while ($row = $mysql->f_obj($sqltest)) {   
                        ...
                        if($row2->tipo == 'PARTICULAR'){
                            ...
                            if($row->debe > '0.00'){
                                ...
                                mod_estatus_pac($row, $pagina);
                                mod_placas($row, $pagina);
                                mod_dicos($row, $pagina);
                                mod_interpretacion($row, $pagina);
                                mod_estatus_interpretacion($row, $pagina);
                                //editar_datos_pac($row);
                                echo '</tr>';
                            }
                            else{
        ...
        <table class="table table-bordered table-hover table-striped table-responsive">
            <thead>
                    ...
                    $sqltest = $mysql->query($link,"SELECT  t1.idpacientes,
                                                            ...
                                                            (SELECT count(idurgencia) 
                                                                FROM urgencias t5
                                                                WHERE t5.idpaciente = t1.idpacientes ) as urgencia,
...
//**************** verifica cantidad de discos placas e interpretaciones hechas ************************
function mod_dicos($row, $pagina){
    //echo $pagina;
    echo'<td>
            <form role="form" id="disco'.$row->idpacientes.'" method="post" action="controlador_discos_placas_interpretacion.php">
                <input type="hidden" form="disco'.$row->idpacientes.'" name = "idpaciente" value="'.$row->idpacientes.'"/>
                <input type="hidden" form="disco'.$row->idpacientes.'" name = "pagina_destino" value="'.$pagina.'"/>
                <input type="hidden" form="disco'.$row->idpacientes.'" name = "nombre" value="'.$row->nombre.'"/>
                <input type="hidden" form="disco'.$row->idpacientes.'" name = "estudio" value="'.$row->estudio.'"/>
                <input type="hidden" form="disco'.$row->idpacientes.'" name = "fecha" value="'.$row->fecha.'"/>';


                //**************    VERIFICO DISCOS ************************
                if($row->discos == NULL){
                    echo '
                        <input type="hidden" form="disco'.$row->idpacientes.'" name = "no_discos" value="0"/>
                        <button type="submit" form="disco'.$row->idpacientes.'" name="editar" class="btn btn-danger btn-default btn-block" >
                            <span class="glyphicon " aria-hidden="true"></span>
                            0 discos
                        </button>';   
                }else
                {
                    echo '
                        <input type="hidden" form="disco'.$row->idpacientes.'" name = "no_discos" value="'.$row->discos.'"/>
                        <button type="submit" form="disco'.$row->idpacientes.'" name="editar" class="btn btn-success btn-default btn-block" >
                            <span class="glyphicon " aria-hidden="true"></span>
                            '.$row->discos.'
                        </button>';
                }
                ...
                if($row->placas == NULL){
                    echo '
                        <input type="hidden" form="disco'.$row->idpacientes.'" name = "no_placas" value="0"/>';
                }
                else{
                    echo '
                         <input type="hidden" form="disco'.$row->idpacientes.'" name = "no_placas" value="'.$row->placas.'"/>';
                }
                //**************    VERIFICO INTERPRETACION ************************

                if($row->interpretacion == NULL){
                    echo '
                        <input type="hidden" form="disco'.$row->idpacientes.'" name = "interpretacion" value="NO"/>';   
                }
                if($row->interpretacion == 'NO'){
                    echo '
                        <input type="hidden" form="disco'.$row->idpacientes.'" name = "interpretacion" value="NO"/>';
                }
                if($row->interpretacion == 'SI'){
                    echo '
                    <input type="hidden" form="disco'.$row->idpacientes.'" name = "interpretacion" value="SI"/>';   
                }
                //**************    estatus  ************************
                if($row->estatus_resultado == NULL){
                    echo '
                        <input type="hidden" form="disco'.$row->idpacientes.'" name = "estatus_resultado" value="NO SE HA ENTREGADO"/>';   
                }
                if($row->estatus_resultado == 'NO SE HA ENTREGADO'){
                    echo '
                        <input type="hidden" form="disco'.$row->idpacientes.'" name = "estatus_resultado" value="NO SE HA ENTREGADO"/>';   
                }
                if($row->estatus_resultado == 'E PLACAS Y DISCOS'){
                    echo '
                        <input type="hidden" form="disco'.$row->idpacientes.'" name = "estatus_resultado" value="E PLACAS Y DISCOS"/>';
                }
                if($row->estatus_resultado == 'ENTREGADO'){
                    echo '
                        <input type="hidden" form="disco'.$row->idpacientes.'" name = "estatus_resultado" value="ENTREGADO"/>';
                }
...
function mod_placas($row, $pagina){
    //echo $pagina;
    echo'<td>
            <form role="form" id="placas'.$row->idpacientes.'" method="post" action="controlador_discos_placas_interpretacion.php">
                <input type="hidden" form="placas'.$row->idpacientes.'" name = "idpaciente" value="'.$row->idpacientes.'"/>
                <input type="hidden" form="placas'.$row->idpacientes.'" name = "pagina_destino" value="'.$pagina.'"/>

                <input type="hidden" form="placas'.$row->idpacientes.'" name = "nombre" value="'.$row->nombre.'"/>
                <input type="hidden" form="placas'.$row->idpacientes.'" name = "estudio" value="'.$row->estudio.'"/>
                <input type="hidden" form="placas'.$row->idpacientes.'" name = "fecha" value="'.$row->fecha.'"/>';

                //**************    VERIFICO DISCOS ************************
                if($row->discos == NULL){
                    echo '
                        <input type="hidden" form="placas'.$row->idpacientes.'" name = "no_discos" value="0"/>';   
                }else
                {
                    echo '
                        <input type="hidden" form="placas'.$row->idpacientes.'" name = "no_discos" value="'.$row->discos.'"/>';
                }

                //**************    VERIFICO PLACAS ************************
                if($row->placas == NULL){
                    echo '
                        <input type="hidden" form="placas'.$row->idpacientes.'" name = "no_placas" value="0"/>
                        <button type="submit" form="placas'.$row->idpacientes.'" name="editar" class="btn btn-danger btn-default btn-block" >
                            <span class="glyphicon " aria-hidden="true"></span>
                            0 placas
                        </button>';   
                }else
                {
                    echo '
                         <input type="hidden" form="placas'.$row->idpacientes.'" name = "no_placas" value="'.$row->placas.'"/>
                        <button type="submit" form="placas'.$row->idpacientes.'" name="editar" class="btn btn-success btn-default btn-block" >
                            <span class="glyphicon " aria-hidden="true"></span>
                            '.$row->placas.'
                        </button>';
                }
                ...
                if($row->interpretacion == NULL){
                    echo '
                        <input type="hidden" form="placas'.$row->idpacientes.'" name = "interpretacion" value="NO"/>';   
                }
                if($row->interpretacion == 'NO'){
                    echo '
                        <input type="hidden" form="placas'.$row->idpacientes.'" name = "interpretacion" value="NO"/>';
                ...
                if($row->interpretacion == 'SI'){
                    echo '
                    <input type="hidden" form="placas'.$row->idpacientes.'" name = "interpretacion" value="SI"/>';   
                }
                //**************    estatus  ************************
                if($row->estatus_resultado == NULL){
                    echo '
                        <input type="hidden" form="placas'.$row->idpacientes.'" name = "estatus_resultado" value="NO SE HA ENTREGADO"/>';   
                }
                if($row->estatus_resultado == 'NO SE HA ENTREGADO'){
                    echo '
                        <input type="hidden" form="placas'.$row->idpacientes.'" name = "estatus_resultado" value="NO SE HA ENTREGADO"/>';   
                ...
                if($row->estatus_resultado == 'E PLACAS Y DISCOS'){
                    echo '
                        <input type="hidden" form="placas'.$row->idpacientes.'" name = "estatus_resultado" value="E PLACAS Y DISCOS"/>';
                }
                if($row->estatus_resultado == 'ENTREGADO'){
                    echo '
                        <input type="hidden" form="placas'.$row->idpacientes.'" name = "estatus_resultado" value="ENTREGADO"/>';
                }
...
function mod_interpretacion($row, $pagina){
    //echo $pagina;
    echo'<td>
            <form role="form" id="interpretacion'.$row->idpacientes.'" method="post" action="controlador_discos_placas_interpretacion.php">
                <input type="hidden" form="interpretacion'.$row->idpacientes.'" name = "idpaciente" value="'.$row->idpacientes.'"/>
                <input type="hidden" form="interpretacion'.$row->idpacientes.'" name = "estatus" value="'.$row->estatus.'"/>
                <input type="hidden" form="interpretacion'.$row->idpacientes.'" name = "pagina_destino" value="'.$pagina.'"/>

                <input type="hidden" form="interpretacion'.$row->idpacientes.'" name = "nombre" value="'.$row->nombre.'"/>
                <input type="hidden" form="interpretacion'.$row->idpacientes.'" name = "estudio" value="'.$row->estudio.'"/>
                <input type="hidden" form="interpretacion'.$row->idpacientes.'" name = "fecha" value="'.$row->fecha.'"/>';
                //**************    VERIFICO DISCOS ************************
                if($row->discos == NULL){
                    echo '
                        <input type="hidden" form="interpretacion'.$row->idpacientes.'" name = "no_discos" value="0"/>';   
                }else
                {
                    echo '
                        <input type="hidden" form="interpretacion'.$row->idpacientes.'" name = "no_discos" value="'.$row->discos.'"/>';
                }

                //**************    VERIFICO PLACAS ************************
                if($row->placas == NULL){
                    echo '
                        <input type="hidden" form="interpretacion'.$row->idpacientes.'" name = "no_placas" value="0"/>';
                }
                else{
                    echo '
                         <input type="hidden" form="interpretacion'.$row->idpacientes.'" name = "no_placas" value="'.$row->placas.'"/>';
                }
                //**************    VERIFICO INTERPRETACION ************************
                if($row->interpretacion == NULL){
                    echo '
                        <button type="submit" form="interpretacion'.$row->idpacientes.'" name="interpretacion" value="NO" class="btn btn-danger btn-default btn-block" >
                            <span class="glyphicon " aria-hidden="true"></span>
                            NO
                        </button>';   
                }
                ...
                {
                    echo '
                        <input type="hidden" form="interpretacion'.$row->idpacientes.'" name = "interpretacion" value="'.$row->interpretacion.'"/>
                        <button type="submit" form="interpretacion'.$row->idpacientes.'" name="editar" class="btn btn-success btn-default btn-block" >
                            <span class="glyphicon " aria-hidden="true"></span>
                            '.$row->interpretacion.'
                        </button>';
                ...
                if($row->estatus_resultado == NULL){
                    echo '
                        <input type="hidden" form="interpretacion'.$row->idpacientes.'" name = "estatus_resultado" value="NO SE HA ENTREGADO"/>';   
                }
                if($row->estatus_resultado == 'NO SE HA ENTREGADO'){
                    echo '
                        <input type="hidden" form="interpretacion'.$row->idpacientes.'" name = "estatus_resultado" value="NO SE HA ENTREGADO"/>';   
                ...
                if($row->estatus_resultado == 'E PLACAS Y DISCOS'){
                    echo '
                        <input type="hidden" form="interpretacion'.$row->idpacientes.'" name = "estatus_resultado" value="E PLACAS Y DISCOS"/>';
                }
                if($row->estatus_resultado == 'ENTREGADO'){
                    echo '
                        <input type="hidden" form="interpretacion'.$row->idpacientes.'" name = "estatus_resultado" value="ENTREGADO"/>';
                }
...
function mod_estatus_interpretacion($row, $pagina){
    //echo $pagina;
    echo'<td>
            <form role="form" id="resultado'.$row->idpacientes.'" method="post" action="controlador_discos_placas_interpretacion.php">
                <input type="hidden" form="resultado'.$row->idpacientes.'" name = "idpaciente" value="'.$row->idpacientes.'"/>
                <input type="hidden" form="resultado'.$row->idpacientes.'" name = "estatus" value="'.$row->estatus.'"/>
                <input type="hidden" form="resultado'.$row->idpacientes.'" name = "pagina_destino" value="'.$pagina.'"/>

                <input type="hidden" form="resultado'.$row->idpacientes.'" name = "nombre" value="'.$row->nombre.'"/>
                <input type="hidden" form="resultado'.$row->idpacientes.'" name = "estudio" value="'.$row->estudio.'"/>
                <input type="hidden" form="resultado'.$row->idpacientes.'" name = "fecha" value="'.$row->fecha.'"/>';
                //**************    VERIFICO DISCOS ************************
                if($row->discos == NULL){
                    echo '
                        <input type="hidden" form="resultado'.$row->idpacientes.'" name = "no_discos" value="0"/>';   
                }else
                {
                    echo '
                        <input type="hidden" form="resultado'.$row->idpacientes.'" name = "no_discos" value="'.$row->discos.'"/>';
                }

                //**************    VERIFICO PLACAS ************************
                if($row->placas == NULL){
                    echo '
                        <input type="hidden" form="resultado'.$row->idpacientes.'" name = "no_placas" value="0"/>';
                }
                else{
                    echo '
                         <input type="hidden" form="resultado'.$row->idpacientes.'" name = "no_placas" value="'.$row->placas.'"/>';
                }
                //**************    VERIFICO INTERPRETACION ************************

                if($row->interpretacion == NULL){
                    echo '
                        <input type="hidden" form="resultado'.$row->idpacientes.'" name = "interpretacion" value="NO"/>';   
                }
                if($row->interpretacion == 'NO'){
                    echo '
                        <input type="hidden" form="resultado'.$row->idpacientes.'" name = "interpretacion" value="NO"/>';
                ...
                if($row->interpretacion == 'SI'){
                    echo '
                    <input type="hidden" form="resultado'.$row->idpacientes.'" name = "interpretacion" value="SI"/>';   
                }

                //**************    estatus  ************************
                if($row->estatus_resultado == NULL){
                    echo '
                        <input type="hidden" form="resultado'.$row->idpacientes.'" name = "estatus_resultado" value="NO SE HA ENTREGADO"/>
                        <button type="submit" form="resultado'.$row->idpacientes.'" name="editar" class="btn btn-danger btn-default btn-block" >
                            <span class="glyphicon " aria-hidden="true"></span>
                           NO SE HA ENTREGADO
                        </button>';   
                }
                ...
                if($row->estatus_resultado == 'E PLACAS Y DISCOS'){
                    echo '
                        <input type="hidden" form="resultado'.$row->idpacientes.'" name = "estatus_resultado" value="E PLACAS Y DISCOS"/>
                        <button type="submit" form="resultado'.$row->idpacientes.'" name="editar" class="btn btn-warning btn-default btn-block" >
                            <span class="glyphicon " aria-hidden="true"></span>
                           E PLACAS Y DISCOS
                        </button>';
                }
                ...
                if($row->estatus_resultado == 'ENTREGADO'){
                    echo '
                        <input type="hidden" form="resultado'.$row->idpacientes.'" name = "estatus_resultado" value="ENTREGADO"/>
                        <button type="submit" form="resultado'.$row->idpacientes.'" name="editar" class="btn btn-success btn-default btn-block" >
                            <span class="glyphicon " aria-hidden="true"></span>
                           ENTREGADO
                        </button>';
                }
                    ...
                    while ($row3 = $mysql->f_obj($sql)) {
                        ...
                        if($row2->tipo == 'PARTICULAR'){
                            ...
                            $sqltest = $mysql->query($link,"SELECT  t1.fecha,
                                                            ...
                                                            (SELECT count(idurgencia) 
                                                                FROM urgencias t6
                                                                WHERE t6.idpaciente = t1.idpacientes ) as urgencia
    ...
    ';
        ...
        $sqltest = $mysql->query($link,"SELECT  t1.pacientes_idpacientes as idpaciente,
            ...
            while ($row = $mysql->f_obj($sqltest)) { 
                ...
                //*************************************************************
                //Selecciona la información a la  recepcionista que corresponde
                //*************************************************************                
                if($_SESSION['id'] == $row->recibio){
...
function reimprimir_recibos($fecha_estudio){
    ...
    ';
                    ...
                    while ($row = $mysql->f_obj($sqltest)) { 
                        ...
                        echo '<td> '.$row->no_recibo.'</td>';
                        echo '<td>
                                <form role="form" id="reimprimir'.$row->idpaciente.'" method="post" action="view_reimprimir_recibo_anticipo.php" target="_blank">
                                    <input type="hidden" form="reimprimir'.$row->idpaciente.'" name="fecha_corte" value="'.$fecha_estudio.'">
                                    <input type="hidden" form="reimprimir'.$row->idpaciente.'" name="idpaciente" value="'.$row->idpaciente.'">
                                    <input type="hidden" form="reimprimir'.$row->idpaciente.'" name="nombre_paciente" value="'.$row->nombre.'">
                                    <input type="hidden" form="reimprimir'.$row->idpaciente.'" name="estudio" value="'.$row->estudio.'">
                                    <input type="hidden" form="reimprimir'.$row->idpaciente.'" name="monto_anticipo" value="'.$monto_anticipo.'">
                                    <input type="hidden" form="reimprimir'.$row->idpaciente.'" name="fecha_anticipo" value="'.$fecha.'">
                        
                                    <button type="submit" form="reimprimir'.$row->idpaciente.'" name="editar" class="btn btn-primary btn-default btn-block" >
                                        <span class="glyphicon glyphicon-print" aria-hidden="true"></span>
                                        Imprimir
                                    </button>
                                </form></td>';
