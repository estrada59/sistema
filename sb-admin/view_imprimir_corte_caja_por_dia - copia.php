	...
	function get_data( $fecha_corte){
        ...
        while ($row = $mysql->f_obj($sqltest)) { 
			...
			if($row->descuento == 100){
				...
				/*if($row->dep_banamex == '0.00'){
					if($row->pago_santander == '0.00'){
						$colum_set = 'Anticipo efectivo';
						$monto_anticipo = $row->anticipo_efe; //es efectivo
					}
					if($row->anticipo_efe == '0.00'){
						$colum_set = 'Pago santander'; 
						$monto_anticipo = $row->pago_santander; // es santander
					}
				}
				else{
						$colum_set = 'Pago banamex'; 
						$monto_anticipo = $row->dep_banamex;//es banamex   
				}*/
